function rx_pulse_mat = downConv(wav, num_pulses, fh_pat)

% 根据跳频图案和一帧中包含的总脉冲个数，将接收的中频信号波形下变频到基带并从64倍符号速率降为4倍符号速率
% 用于2Mbps A模式

% 参数定义
bit_rate = 16e6;  % 符号速率
T = 1/bit_rate;  % 符号时间
fs_IF = 1024e6;  % 中频信号采样速率
fs_BB = 64e6;  % 基带信号采样速率
oversamp_BB = T * fs_BB;  % 基带信号过采样速率
oversamp_IF = T * fs_IF;  % 中频信号过采样速率
num_bits_pulse = 304;  % 2Mbps A\500Kbps\250Kbps 一个脉冲包的长度
                       % 2Mbps B 每个脉冲去掉前后跳数据部分后的长度

% 低通滤波器
load('filter/LPF_1.mat'); % 采样率1024MHz，通带20MHz
load('filter/LPF_2.mat'); % 采样率256MHz，通带10MHz
S_lpf1 = 32;
S_lpf2 = 32;

% 21个频点下变频至零中频所需各点频率
f_chan12_1 = 213.34e6;  % 通道1、2 第1个频点对应的频率
f_chan12_2 = 226.67e6;  % 通道1、2 第2个频点对应的频率
f_chan12_3 = 240e6;     % 通道1、2 第3个频点对应的频率
f_chan12_4 = 253.33e6;  % 通道1、2 第4个频点对应的频率
f_chan12_5 = 266.66e6;  % 通道1、2 第5个频点对应的频率
f_chan34_1 = 220.005e6; % 通道3、4 第1个频点对应的频率
f_chan34_2 = 233.335e6; % 通道3、4 第2个频点对应的频率
f_chan34_3 = 246.665e6; % 通道3、4 第3个频点对应的频率
f_chan34_4 = 259.995e6; % 通道3、4 第4个频点对应的频率
f_chan5_1 = f_chan12_2; % 通道5 第1个频点对应的频率
f_chan5_2 = f_chan12_3; % 通道5 第2个频点对应的频率
f_chan5_3 = f_chan12_4; % 通道5 第3个频点对应的频率

t_240_chip = -num_bits_pulse*T/2:1/fs_IF:num_bits_pulse*T/2-1/fs_IF;
t_240_pulse = t_240_chip(1:end) + (T/oversamp_IF/2);  % 中心对称;

rx_pulse_mat = zeros(num_pulses, num_bits_pulse * oversamp_BB);
for pulse_idx = 1:num_pulses
    
    f_idx = fh_pat(pulse_idx);  % 按脉冲序号和跳频图案，找到当前脉冲对应的频点
    
    switch f_idx % 根据频点序号做对应的滤波
                        
        % 变换至零中频
        case 1  % 频点1
            rx_f1_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_1*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f1_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 2  % 频点2
            rx_f2_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_2*t_240_pulse)); % 取差频
            rx_f2_256 = downsample(rx_f2_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f2_64 = downsample(rx_f2_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f2_64;



        case 3  % 频点3
            rx_f3_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_3*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f3_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 4  % 频点4
            rx_f4_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_4*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f4_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 5  % 频点5
            rx_f5_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_5*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f5_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 6  % 频点6                            
            rx_f6_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_1*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f6_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 7  % 频点7
            rx_f7_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_2*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f7_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 8  % 频点8    
            rx_f8_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_3*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f8_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;
     


        case 9  % 频点9  
            rx_f9_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_4*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f9_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 10 % 频点10   
            rx_f10_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan12_5*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f10_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 11 % 频点11                           
            rx_f11_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_1*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f11_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 12 % 频点12
            rx_f12_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_2*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f12_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 13 % 频点13
            rx_f13_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_3*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f13_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 14 % 频点14
            rx_f14_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_4*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f14_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 15 % 频点15
            rx_f15_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_1*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f15_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 16 % 频点16
            rx_f16_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_2*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f16_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 17 % 频点17
            rx_f17_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_3*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f17_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;
                            


        case 18 % 频点18
            rx_f18_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan34_4*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f18_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 19 % 频点19
            rx_f19_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan5_1*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f19_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;
                          


        case 20 % 频点20
            rx_f20_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan5_2*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f20_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;



        case 21 % 频点21
            rx_f21_temp1 = wav(pulse_idx,:) .* exp(1i*(-2*pi*f_chan5_3*t_240_pulse)); % 取差频
            rx_f1_256 = downsample(rx_f21_temp1(1:304*oversamp_IF), 4);  % 4倍抽取， 采样率降为256（3是偏移量）
            rx_f1_64 = downsample(rx_f1_256(1:304*oversamp_IF/4), 4);  % 4倍抽取， 采样率降为64（3是偏移量）
            rx_pulse_mat(pulse_idx,:) = rx_f1_64;
  
    end
                
    
end