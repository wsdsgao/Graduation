...
for pulse_idx = 1:num_pulses
    ...
    switch f_idx % 根据频点序号做对应的滤波
        ...
        case 1  % 频点1
            ...
            rx_f1_128_temp1 = conv(rx_f1_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f1_128_temp2 = conv(rx_f1_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f1_128_1 = downsample(rx_f1_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128 
        ...
        case 2  % 频点2
            ...
            rx_f2_128_temp1 = conv(rx_f2_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f2_128_temp2 = conv(rx_f2_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f2_128_1 = downsample(rx_f2_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128  
        ...
        case 3  % 频点3
            ...
            rx_f3_128_temp1 = conv(rx_f3_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f3_128_temp2 = conv(rx_f3_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f3_128_1 = downsample(rx_f3_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128  
        ...
        case 4  % 频点4
            ...
            rx_f4_128_temp1 = conv(rx_f4_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f4_128_temp2 = conv(rx_f4_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f4_128_1 = downsample(rx_f4_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128  
        ...
        case 5  % 频点5
            ...
            rx_f5_128_temp1 = conv(rx_f5_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f5_128_temp2 = conv(rx_f5_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f5_128_1 = downsample(rx_f5_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128
        ...
        case 6  % 频点6                            
            ...
            rx_f6_128_temp1 = conv(rx_f6_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f6_128_temp2 = conv(rx_f6_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f6_128_1 = downsample(rx_f6_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128        
        ...
        case 7  % 频点7
            ...
            rx_f7_128_temp1 = conv(rx_f7_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f7_128_temp2 = conv(rx_f7_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f7_128_1 = downsample(rx_f7_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128        
        ...
        case 8  % 频点8    
            ...
            rx_f8_128_temp1 = conv(rx_f8_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f8_128_temp2 = conv(rx_f8_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f8_128_1 = downsample(rx_f8_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128        
        ...
        case 9  % 频点9  
            ...
            rx_f9_128_temp1 = conv(rx_f9_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f9_128_temp2 = conv(rx_f9_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f9_128_1 = downsample(rx_f9_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128  
        ...
        case 10 % 频点10   
            ...
            rx_f10_128_temp1 = conv(rx_f10_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f10_128_temp2 = conv(rx_f10_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f10_128_1 = downsample(rx_f10_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128   
        ...
        case 11 % 频点11                           
            ...
            rx_f11_128_temp1 = conv(rx_f11_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f11_128_temp2 = conv(rx_f11_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f11_128_1 = downsample(rx_f11_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128   
        ...
        case 12 % 频点12
            ...
            rx_f12_128_temp1 = conv(rx_f12_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f12_128_temp2 = conv(rx_f12_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f12_128_1 = downsample(rx_f12_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128
        ...
        case 13 % 频点13
            ...
            rx_f13_128_temp1 = conv(rx_f13_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f13_128_temp2 = conv(rx_f13_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f13_128_1 = downsample(rx_f13_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128 
        ...
        case 14 % 频点14
            ...
            rx_f14_128_temp1 = conv(rx_f14_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f14_128_temp2 = conv(rx_f14_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f14_128_1 = downsample(rx_f14_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128        
        ...
        case 15 % 频点15
            ...
            rx_f15_128_temp1 = conv(rx_f15_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f15_128_temp2 = conv(rx_f15_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f15_128_1 = downsample(rx_f15_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128        
        ...
        case 16 % 频点16
            ...
            rx_f16_128_temp1 = conv(rx_f16_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f16_128_temp2 = conv(rx_f16_temp1(259*oversamp_IF+1:end), LPF_2);    % 低通滤波(后45chip)
            rx_f16_128_1 = downsample(rx_f16_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128
        ...
        case 17 % 频点17
            ...
            rx_f17_128_temp1 = conv(rx_f17_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f17_128_temp2 = conv(rx_f17_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f17_128_1 = downsample(rx_f17_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128        
        ...
        case 18 % 频点18
            ...
            rx_f18_128_temp1 = conv(rx_f18_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f18_128_temp2 = conv(rx_f18_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f18_128_1 = downsample(rx_f18_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128        
        ...
        case 19 % 频点19
            ...
            rx_f19_128_temp1 = conv(rx_f19_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f19_128_temp2 = conv(rx_f19_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f19_128_1 = downsample(rx_f19_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128      
        ...
        case 20 % 频点20
            ...
            rx_f20_128_temp1 = conv(rx_f20_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f20_128_temp2 = conv(rx_f20_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f20_128_1 = downsample(rx_f20_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128
        ...
        case 21 % 频点21
            ...
            rx_f21_128_temp1 = conv(rx_f21_temp1(1:259*oversamp_IF), LPF_2);  % 低通滤波(前259chip)
            rx_f21_128_temp2 = conv(rx_f21_temp1(259*oversamp_IF+1:end), LPF_2);  % 低通滤波(后45chip)
            rx_f21_128_1 = downsample(rx_f21_128_temp1(S_lpf2+1:S_lpf2+259*oversamp_IF), 8, 7);  % 8倍抽取， 采样率降为128
