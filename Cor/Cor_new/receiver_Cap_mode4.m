...
D_S1_S3_one = D_S1_S3(:,4:oversamp_BB:end);
D_S4_S2 = rx_pulse_mat_FNL(:,259*oversamp_BB+1:304*oversamp_BB);
D_S4_S2_one = D_S4_S2(:,4:oversamp_BB:end);
...
DF_hat_S12 = atan2(-imag(mean(zk_corr_S12)), real(mean(zk_corr_S12)))/(2*pi*259*T);

% 同步头消除频偏
counter_f_S1 = repmat(DF_hat_S12,[num_pulses,1]) * 2 * pi * t2(16:16:45*oversamp_IF) * T;
counter_f_S2 = repmat(DF_hat_S12,[num_pulses,1]) * 2 * pi * t2(259*oversamp_IF+16:16:304*oversamp_IF) * T;
...
for j = 1:num_pulses
    delta_theta_cplx_mat_S1 = D_S1_S3_half(j,4:oversamp_BB:end) .* conj(wav_S1_S3(j,:));
    delta_theta_cplx_mat_S2 = D_S4_S2_half(j,4:oversamp_BB:end) .* conj(wav_S4_S2(j,:));
    delta_theta_cplx = (sum(delta_theta_cplx_mat_S1(1:43)) + sum(delta_theta_cplx_mat_S2(1:43)))/(43*2); %用43个？
...
rx_pulse_mat_FNL1 = zreos(num_pulses, 214*oversamp_BB);
rx_pulse_mat_FNL3 = rx_pulse_mat_FNL(:, 45*oversamp_BB+1:259*oversamp_BB);
