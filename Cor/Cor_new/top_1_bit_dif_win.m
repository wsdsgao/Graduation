...
path_record = cell(64, 2);
path_weight = zeros(64, 2);
signal_recv_dif = signal_recv_BB(1:4) .* conj(signal_recv_BB(5:8));
for q = 1:16
...
for n = 2:num_bits_pulse
    ...
    signal_recv_dif = signal_recv_BB(1+(n-1)*4:n*4) .* conj(signal_recv_BB(1+n*4:(n+1)*4));
    ...
    path_weight(:,2) = zeros(64,1);

    if n == num_bits_pulse
        [~,index_max] = max(path_weight(:,1));
        out = path_record{index_max,1};
        de_out(de_out_index : end) = out(3:end-3);
    else
