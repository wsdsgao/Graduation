...
signal_recv_BB = [signal_recv_BB, zeros(1,3*oversamp_IF)];
de_out_index = 1;

de_out = zeros(size(I));
viterbi_deep = 80;
% signal_trans_IF(1 : oversamp_IF*viterbi_deep)
path_record = cell(256, 2);
path_weight = zeros(256, 2);
signal_recv_dif = signal_recv_BB(1:4) .* conj(signal_recv_BB(13:16));
for q = 1:64
    init_path = dec2bin(q-1, 6) - '0';
    init_path = [0,0,init_path];
    path_record{q,1} = init_path;
    bit_5_fst = init_path(end-7:end-3);
    bit_5_sec = init_path(end-6:end-2);
    bit_5_trd = init_path(end-5:end-1);
    bit_5_for = init_path(end-4:end);
    [phi_last, I_sig_fst, Q_sig_fst, ~] = GMSK(2*bit_5_fst-1, 0, g);
    [phi_last, ~, ~, ~] = GMSK(2*bit_5_sec-1, phi_last, g);
    [phi_last, ~, ~, ~] = GMSK(2*bit_5_trd-1, phi_last, g);
    [~, I_sig_for, Q_sig_for, ~] = GMSK(2*bit_5_for-1, phi_last, g);
    g_path_dif = complex(I_sig_fst, Q_sig_fst) .* conj(complex(I_sig_for, Q_sig_for));
    path_weight(q, 1) = real(sum(signal_recv_dif .* conj(g_path_dif))); % 取最大值
end  

for n = 2:num_bits_pulse
    
    signal_recv_dif = signal_recv_BB(1+(n-1)*4:n*4) .* conj(signal_recv_BB(1+(n+2)*4:(n+3)*4));
    ...
    for j = 1 : 256
        if ~isempty(path_record{j,1})
            ...
            bit_5_fst0 = bit_record_cur0(end-7:end-3);
            bit_5_sec0 = bit_record_cur0(end-6:end-2);
            bit_5_trd0 = bit_record_cur0(end-5:end-1);
            bit_5_for0 = bit_record_cur0(end-4:end);

            bit_5_fst1 = bit_record_cur1(end-7:end-3);
            bit_5_sec1 = bit_record_cur1(end-6:end-2);
            bit_5_trd1 = bit_record_cur1(end-5:end-1);
            bit_5_for1 = bit_record_cur1(end-4:end);
            
            [phi_last0, I_sig0_fst, Q_sig0_fst, ~] = GMSK(2*bit_5_fst0-1, 0, g);
            [phi_last0, ~, ~, ~] = GMSK(2*bit_5_sec0-1, phi_last0, g);
            [phi_last0, ~, ~, ~] = GMSK(2*bit_5_trd0-1, phi_last0, g);
            [~, I_sig0_for, Q_sig0_for, ~] = GMSK(2*bit_5_for0-1, phi_last0, g);

            [phi_last1, I_sig1_fst, Q_sig1_fst, ~] = GMSK(2*bit_5_fst1-1, 0, g);
            [phi_last1, ~, ~, ~] = GMSK(2*bit_5_sec1-1, phi_last1, g);
            [phi_last1, ~, ~, ~] = GMSK(2*bit_5_trd1-1, phi_last1, g);
            [~, I_sig1_for, Q_sig1_for, ~] = GMSK(2*bit_5_for1-1, phi_last1, g);
            
            state_index0 = bin2dec(num2str(bit_record_cur0(end-7:end)))+1;
            state_index1 = bin2dec(num2str(bit_record_cur1(end-7:end)))+1;
            
            g_path_dif0 = complex(I_sig0_fst, Q_sig0_fst) .* conj(complex(I_sig0_for, Q_sig0_for));
            g_path_dif1 = complex(I_sig1_fst, Q_sig1_fst) .* conj(complex(I_sig1_for, Q_sig1_for));
            
            path_weight0 = real(sum(signal_recv_dif .* conj(g_path_dif0)));
            path_weight1 = real(sum(signal_recv_dif .* conj(g_path_dif1)));

            if path_weight(state_index0,2) == 0 || path_weight(j,1) + path_weight0 > path_weight(state_index0,2)
                path_weight(state_index0,2) =  path_weight(j,1) + path_weight0;
                path_record{state_index0, 2} = bit_record_cur0;                   
            end
            
            if path_weight(state_index1,2) == 0 || path_weight(j,1) + path_weight1 > path_weight(state_index1,2)
                path_weight(state_index1,2) =  path_weight(j,1) + path_weight1;
    ...
    path_weight(:,1) = path_weight(:,2);
    path_weight(:,2) = zeros(256,1);

    if n == num_bits_pulse
        [~,index_max] = max(path_weight(:,1));
        out = path_record{index_max,1};
        de_out(de_out_index : end) = out(3:end-5);
    else
