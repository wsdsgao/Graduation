function [phi, I_sig_BB, Q_sig_BB, int_phi] = GMSK(bit_5, f_c, phi_last, g)
% input: 5比特数据              bit_5 
%        当前跳频载波            fc 
%        上一bit末相位          phi_last(即bit_5的第三个bit值)(不准确？)
%        g                    对应BT=0.3的g函数
...
t = t(1:end) + (T/oversamp_IF/2);  % 中心对称（加上采样时间的一半）
int_phi = mod(pi/(2*T).*intphi, 2*pi);

phi = int_phi(T*fs_IF)/(pi/(2*T));  %末相位

% I路信号
I_sig_BB = cos(int_phi);
I_sig_IF = cos(2*pi*f_c.*t);  %没有用
