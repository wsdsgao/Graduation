function result = receiver(rx, fh_pat_lib_1, th_pat_lib_1, fh_pat_lib_2, th_pat_lib_2, ...
    wav_S1_1_mode1, wav_S1_2_mode1, wav_S2_1_mode1, wav_S2_2_mode1, ...
    wav_S1_1_mode2, wav_S1_2_mode2, wav_S2_1_mode2, wav_S2_2_mode2, ...
    wav_S1_S3_1_mode3, wav_S4_S2_1_mode3, wav_S1_S3_2_mode3, wav_S4_S2_2_mode3, ...
    wav_S1_S3_1_mode4, wav_S4_S2_1_mode4, wav_S1_S3_2_mode4, wav_S4_S2_2_mode4, ...
    frame_num)
...
for i = 1:length(rx)-time_frame_mode2*oversamp_IF   % 等待一帧的时间长度
     ...
     % for mode1 
        ...
        % 两个连续10ms的图案
        wav_temp_mode1 = zeros(num_pulses_mode1, num_bits_pulse * oversamp_IF ,2);
        % 第1个跳时跳频图案
        for pulse_idx = 1:num_pulses_mode1
            f_idx = fh_pat_1_mode1(pulse_idx); % 当前脉冲对应频点
            
            % 计算当前脉冲在一帧中的起始位置
            if pulse_idx == 1
                pos_pulse_mode1 = th_pat_1_mode1(1)/2;
            else
            ...
            wav_temp_mode1(pulse_idx,:,1) = wav_temp1_mode1;
        ...
        % 第2个跳时跳频图案
        for pulse_idx = 1:num_pulses_mode1
            f_idx = fh_pat_2_mode1(pulse_idx);  % 当前脉冲对应的频点
            
            % 计算当前脉冲在一帧中的起始位置
            if pulse_idx == 1
                pos_pulse_mode1 = th_pat_2_mode1(1)/2;
            else
                pos_pulse_mode1 = sum(th_pat_2_mode1(1:pulse_idx-1)) + (pulse_idx-1)*num_bits_pulse + th_pat_2_mode1(pulse_idx)/2;
            end
            ...
            wav_temp_mode1(pulse_idx,:,2) = wav_temp1_mode1;
        ...
        rx_pulse_mat_1_mode1 = downConv(wav_temp_mode1(:,:,1), num_pulses_mode1, fh_pat_1_mode1);  % 对应前10ms
        ...
        % 求前后同步头相关峰
        rx_corr_S1_pat_1_mode1 = zeros(1,num_pulses_mode1);
        rx_corr_S2_pat_1_mode1 = zeros(1,num_pulses_mode1);
        for j = 1:num_pulses_mode1

            rx_wav_S1_pat_1_mode1 = D_S1_1_one_mode1(j,1:22) .* conj(wav_S1_1_mode1(j,1:22));
            rx_wav_S2_pat_1_mode1 = D_S2_1_one_mode1(j,1:22) .* conj(wav_S2_1_mode1(j,1:22));   
        ...
        % 同步捕获判决条件
        % 前(或后)同步头相关峰值大于阈值
        if (sum(rx_corr_S1_pat_1_mode1)>20*num_pulses_mode1) || (sum(rx_corr_S2_pat_1_mode1)>20*num_pulses_mode1)
            mode_sel = 1;
            flag_Capture_C = 1;
    ...
    % for mode2
        ...
        for pulse_idx = 1:num_pulses_mode2
            ...
            % 计算当前脉冲的长度
            num_bits_pulse_rx = th_pat_1_mode2(pulse_idx) + 3 + num_bits_pulse;
        ...
        % 根据跳频图案对应的频点将对应通道的波形滤波至零中频
        % wav_temp(采样率1024) -> rx_pulse_mat(采样率128)
        rx_pulse_mat_1_mode2 = downConv_2MLL(wav_temp_mode2(:,:,1), num_pulses_mode2, fh_pat_1_mode2, th_pat_1_mode2);  % 对应前10ms
        ...
        % 前10ms
        D_S1_1_mode2 = rx_pulse_mat_1_mode2(:,1:num_bits_pn*oversamp_BB);
        D_S1_1_one_mode2 = D_S1_1_mode2(:,8:oversamp_BB:end);
        D_S2_1_mode2 = rx_pulse_mat_1_mode2(:,2240+1:2240+num_bits_pn*oversamp_BB);
        D_S2_1_one_mode2 = D_S2_1_mode2(:,8:oversamp_BB:end);
        % 后10ms
        ...
        for j = 1:num_pulses_mode2
         ...
         % 同步捕获判决条件
         % 前(或后)同步头相关峰值大于阈值       
         if (sum(rx_corr_S1_pat_1_mode2)>20*num_pulses_mode2) || (sum(rx_corr_S2_pat_1_mode2)>20*num_pulses_mode2) 
            mode_sel = 2;
            flag_Capture_C = 1; 
    ...
    % for mode3
        ...
        % 根据跳频图案对应的频点找到对应通道的波形
        % 两个连续10ms的图案
        wav_temp_mode3 = zeros(num_pulses_mode3, num_bits_pulse * oversamp_IF ,2);
        % 第1个跳时跳频图案
        for pulse_idx = 1:num_pulses_mode3
            f_idx = fh_pat_1_mode3(pulse_idx);  % 当前脉冲对应的频点
            
            % 计算当前脉冲在一帧中的起始位置
            if pulse_idx == 1
                pos_pulse_mode3 = th_pat_1_mode3(1)/2;
            else
                pos_pulse_mode3 = sum(th_pat_1_mode3(1:pulse_idx-1)) + (pulse_idx-1)*num_bits_pulse + th_pat_1_mode3(pulse_idx)/2;
            end
            ...
            wav_temp_mode3(pulse_idx,:,1) = wav_temp1_mode3;
        ...
        % 根据跳频图案对应的频点将对应通道的波形滤波至零中频
        % wav_temp(采样率1024) -> rx_pulse_mat(采样率128)
        rx_pulse_mat_1_mode3 = downConv_500K(wav_temp_mode3(:,:,1), num_pulses_mode3, fh_pat_1_mode3);  % 对应前10ms
        rx_pulse_mat_2_mode3 = downConv_500K(wav_temp_mode3(:,:,2), num_pulses_mode3, fh_pat_2_mode3);  % 对应后10ms


        % 预取前后24bit位置的波形 等待后续处理
        % 前10ms
        D_S1_1_mode3 = rx_pulse_mat_1_mode3(:,1:24*oversamp_BB);
        D_S1_1_one_mode3 = D_S1_1_mode3(:,8:oversamp_BB:end);
        D_S2_1_mode3 = rx_pulse_mat_1_mode3(:,280*oversamp_BB+1:304*oversamp_BB);
        D_S2_1_one_mode3 = D_S2_1_mode3(:,8:oversamp_BB:end);
        D_S3_1_mode3 = rx_pulse_mat_1_mode3(:,24*oversamp_BB+1:45*oversamp_BB);
        D_S3_1_one_mode3 = D_S3_1_mode3(:,8:oversamp_BB:end);
        D_S4_1_mode3 = rx_pulse_mat_1_mode3(:,259*oversamp_BB+1:280*oversamp_BB);
        D_S4_1_one_mode3 = D_S4_1_mode3(:,8:oversamp_BB:end);
        ...
        for j = 1:num_pulses_mode3
         ...
         if     ((sum(rx_corr_S1_pat_1_mode3)>22*num_pulses_mode3) && (sum(rx_corr_S3_pat_1_mode3)>17*num_pulses_mode3))  ...
                 || ((sum(rx_corr_S2_pat_1_mode3)>20*num_pulses_mode3) && (sum(rx_corr_S4_pat_1_mode3)>19*num_pulses_mode3)) 
             flag_Capture_C = 1;
             mode_sel = 3;
    ...
    % for mode4
        ...
        % 根据跳频图案对应的频点找到对应通道的波形
        % 两个连续10ms的图案
        wav_temp_mode4 = zeros(num_pulses_mode4, num_bits_pulse * oversamp_IF ,2);
        % 第1个跳时跳频图案
        for pulse_idx = 1:num_pulses_mode4
            f_idx = fh_pat_1_mode4(pulse_idx);  % 当前脉冲对应的频点
            
            % 计算当前脉冲在一帧中的起始位置
            if pulse_idx == 1
                pos_pulse_mode4 = th_pat_1_mode4(1)/2;
            else
                pos_pulse_mode4 = sum(th_pat_1_mode4(1:pulse_idx-1)) + (pulse_idx-1)*num_bits_pulse + th_pat_1_mode4(pulse_idx)/2;
            end
            ...
            wav_temp_mode4(pulse_idx,:,1) = wav_temp1_mode4;
        ...
        % 根据跳频图案对应的频点将对应通道的波形滤波至零中频
        % wav_temp(采样率1024) -> rx_pulse_mat(采样率128)
        rx_pulse_mat_1_mode4 = downConv_500K(wav_temp_mode4(:,:,1), num_pulses_mode4, fh_pat_1_mode4);  % 对应前10ms
        rx_pulse_mat_2_mode4 = downConv_500K(wav_temp_mode4(:,:,2), num_pulses_mode4, fh_pat_2_mode4);  % 对应后10ms


        % 预取前后24bit位置的波形 等待后续处理
        % 前10ms
        D_S1_1_mode4 = rx_pulse_mat_1_mode4(:,1:24*oversamp_BB);
        D_S1_1_one_mode4 = D_S1_1_mode4(:,8:oversamp_BB:end);
        D_S2_1_mode4 = rx_pulse_mat_1_mode4(:,280*oversamp_BB+1:304*oversamp_BB);
        D_S2_1_one_mode4 = D_S2_1_mode4(:,8:oversamp_BB:end);
        D_S3_1_mode4 = rx_pulse_mat_1_mode4(:,24*oversamp_BB+1:45*oversamp_BB);
        D_S3_1_one_mode4 = D_S3_1_mode4(:,8:oversamp_BB:end);
        D_S4_1_mode4 = rx_pulse_mat_1_mode4(:,259*oversamp_BB+1:280*oversamp_BB);
        D_S4_1_one_mode4 = D_S4_1_mode4(:,8:oversamp_BB:end);
        ...
        for j = 1:num_pulses_mode4
         ...
         if     ((sum(rx_corr_S1_pat_1_mode4)>22*num_pulses_mode4) && (sum(rx_corr_S3_pat_1_mode4)>17*num_pulses_mode4)) ...
                 || ((sum(rx_corr_S2_pat_1_mode4)>20*num_pulses_mode4) && (sum(rx_corr_S4_pat_1_mode4)>19*num_pulses_mode4)) 
             flag_Capture_C = 1;
             mode_sel = 4;
    ...
    if (flag_Capture_C == 1)
        if mode_sel == 1
            ...
            if tag == 1 || tag == 2             % 确定跳频\跳时及同步头图案
                fh_pat_rec = fh_pat_1_mode1;
                th_pat_rec = th_pat_1_mode1;
                wav_S1_rec = wav_S1_1_mode1;    
                wav_S2_rec = wav_S2_1_mode1;    
            elseif tag == 3 || tag == 4
            ...
            time_frame = time_frame_mode1;  % 一帧的总长度(含跳时)
            rx_Cap = rx(i:i+time_frame*oversamp_IF+oversamp_IF*2);
            result_frame = receiver_Cap_mode1(rx_Cap, fh_pat_rec, th_pat_rec, wav_S1_rec, wav_S2_rec, tag, time_frame, num_pulses_rec);  % 送入2Mbps A模式对应的解调模块解调
        ...
        elseif mode_sel == 2
            ...
            if tag == 1 || tag == 2             % 确定跳频\跳时及同步头图案
                ...
                wav_S1_rec = wav_S1_1_mode2;    
                wav_S2_rec = wav_S2_1_mode2;    
            elseif tag == 3 || tag == 4
        ...
        elseif mode_sel == 3
            ...
            if tag == 1 || tag == 2             % 确定跳频\跳时及同步头图案
                ...
                wav_S1_rec = wav_S1_S3_1_mode3;    
                wav_S2_rec = wav_S4_S2_1_mode3;    
            elseif tag == 3 || tag == 4
            ...
            rx_Cap = rx(i:i+time_frame*oversamp_IF+oversamp_IF*2);
            result_frame = receiver_Cap_mode3(rx_Cap, fh_pat_rec, th_pat_rec, wav_S1_rec, wav_S2_rec, tag, time_frame, num_pulses_rec);   % 送入500Kbps模式对应的解调模块解调
            ...
            if tag == 1 || tag == 2              % 确定跳频\跳时及同步头图案
                ...
                wav_S1_rec = wav_S1_S3_1_mode4;    
                wav_S2_rec = wav_S4_S2_1_mode4;    
            elseif tag == 3 || tag == 4
            ...
            rx_Cap = rx(i:i+time_frame*oversamp_IF+oversamp_IF*2);
            result_frame = receiver_Cap_mode3(rx_Cap, fh_pat_rec, th_pat_rec, wav_S1_rec, wav_S2_rec, tag, time_frame, num_pulses_rec);   % 送入250Kbps模式对应的解调模块解调
            result(ceil((i+(time_frame-0.5)*oversamp_IF)/(time_frame*oversamp_IF)), 1:length_frame) = result_frame;  % 记录解调结果
