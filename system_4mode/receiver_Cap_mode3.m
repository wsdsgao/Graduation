function result = receiver_Cap_mode3(rx, fh_pat, th_pat, wav_S1_S3, wav_S4_S2, tag, time_frame, num_pulses)

% 500Kbps\250Kbps模式 的解调模块
...
Wav_str_Cap_mat = zeros(8, time_frame*oversamp_IF);
% 间隔8个采样点提取一路波形，共8路
for offset = 1:nn
...
for offset = 1:nn
    ...
    for pulse_idx = 1:num_pulses
        ...
        if pulse_idx == 1
            ...
            pos_pulse = sum(th_pat(1:pulse_idx-1)) + (pulse_idx-1)*num_bits_pulse + th_pat(pulse_idx)/2;
        end
    ...
    D_S1_S3_one = D_S1_S3(:,8:oversamp_BB:end);
    D_S4_S2 = rx_pulse_mat(:,259*oversamp_BB+1:304*oversamp_BB);
    D_S4_S2_one = D_S4_S2(:,8:oversamp_BB:end);


    % 同步头捕获精细校准                
    % 利用本地的PN波形做互相关 
    rx_corr_S1_S3 = zeros(1,num_pulses);
    rx_corr_S4_S2 = zeros(1,num_pulses);              
    for j = 1:num_pulses

        rx_wav_S1_S3 = D_S1_S3_one(j,1:43) .* conj(wav_S1_S3(j,1:43));  % S1+S3 43bit
        rx_wav_S4_S2 = D_S4_S2_one(j,1:43) .* conj(wav_S4_S2(j,1:43));  % S4+S2 43bit  

        rx_corr_S1_S3(j) = abs(sum(rx_wav_S1_S3));
        rx_corr_S4_S2(j) = abs(sum(rx_wav_S4_S2));

    end

    Corr_S1_S3(offset) = sum(rx_corr_S1_S3);
    Corr_S4_S2(offset) = sum(rx_corr_S4_S2);
...
Corr_S1_S3_Cap_F = zeros(1, 9);
Corr_S4_S2_Cap_F = zeros(1, 9);           
for offset = 1:9
    ...
    % 根据跳频图案对应的频点将对应通道的波形滤波至零中频
    % wav_temp(采样率1024) -> rx_pulse_mat(采样率128)
    rx_pulse_mat = downConv_500K(Wav_str_Cap_F_temp2(:,:,offset), num_pulses, fh_pat);  % 对应前10ms
...
if (tag == 1) || (tag == 3)
    ...
    offset_i = pos_Corr_S1_S3
    offset_j = pos_Corr_S1_S3_F
    choose_flag = 1;

elseif (tag == 2) || (tag == 4)

    offset_i = pos_Corr_S4_S2
...
% 根据跳频图案对应的频点将对应通道的波形滤波至零中频
% wav_temp(采样率1024) -> rx_pulse_mat(采样率128)
rx_pulse_mat_FNL = downConv_500K(wav_temp_FNL(:,:), num_pulses, fh_pat);  

% 预取前后24bit位置的波形 等待后续处理
% 前10ms
D_S1_S3 = rx_pulse_mat_FNL(:,1:45*oversamp_BB);
D_S1_S3_one = D_S1_S3(:,8:oversamp_BB:end);
D_S4_S2 = rx_pulse_mat_FNL(:,259*oversamp_BB+1:304*oversamp_BB);
D_S4_S2_one = D_S4_S2(:,8:oversamp_BB:end);

zk_S1 = zeros(num_pulses, (num_bits_pn+num_bits_pn_2-2));
zk_S2 = zeros(num_pulses, (num_bits_pn+num_bits_pn_2-2));
for j = 1:num_pulses                  
    zk_S1(j,:) = D_S1_S3_one(j,1:43) .* conj(wav_S1_S3(j,1:43));
    zk_S2(j,:) = D_S4_S2_one(j,1:43) .* conj(wav_S4_S2(j,1:43)); 
end
...
for j = 1:num_pulses 
    ...
    delta_f_hat_S12(j) = deltat_hat_S12(j)/(2*pi*259*T);

end
DF_hat_S12 = mean(delta_f_hat_S12);

% 同步头消除频偏
counter_f_S1 = repmat(DF_hat_S12,[num_pulses,1]) * 2 * pi * t2(8:oversamp_BB:45*oversamp_IF) * T;
counter_f_S2 = repmat(DF_hat_S12,[num_pulses,1]) * 2 * pi * t2(259*oversamp_IF+8:oversamp_BB:304*oversamp_IF) * T;
D_S1_S3_half = D_S1_S3 .* complex(cos(counter_f_S1), sin(counter_f_S1));  % 对前后同步头位置的波形消除频偏
D_S4_S2_half = D_S4_S2 .* complex(cos(counter_f_S2), sin(counter_f_S2));

% 再次利用本地的PN波形计算相偏
delta_theta_S1 = zeros(num_pulses, 1);
delta_theta_S2 = zeros(num_pulses, 1);
for j = 1:num_pulses
    delta_theta_cplx_mat_S1 = D_S1_S3_half(j,8:oversamp_BB:end) .* conj(wav_S1_S3(j,:));
    delta_theta_cplx_mat_S2 = D_S4_S2_half(j,8:oversamp_BB:end) .* conj(wav_S4_S2(j,:));   
    delta_theta_cplx_S1 = sum(delta_theta_cplx_mat_S1(1:43)) / 43;
    delta_theta_cplx_S2 = sum(delta_theta_cplx_mat_S2(1:43)) / 43;
...
% 对当前帧的波形利用之前计算所得的频偏及相偏估计值做波形校正，准备送入解调模块（8倍速率）
counter_f = delta_f * 2 * pi * t2(8:oversamp_BB:end) * T;
...
% GMSK 解调（8倍速率）
out_temp = zeros(num_pulses, 258);
for pulse_idx = 1:num_pulses
    out_temp(pulse_idx,:) = GMSK_demod(rx_pulse_mat_FNL(pulse_idx, 1:(24+21+214)*oversamp_BB), c0_f, c1_f, oversamp_BB, 1, 260);
end

% 解调结果组成完整的一帧
for pulse_idx = 1:num_pulses
    result((pulse_idx-1)*214+1:pulse_idx*214) = out_temp(pulse_idx, 45:end);
end
