...
for pulse_idx = 1:num_pulses
    ...
    Wav_store_FNL(pulse_idx, 1:(th_pat(pulse_idx))*oversamp_IF) = [wav_temp1(1:(th_pat(pulse_idx)/2)*oversamp_IF), wav_temp1((th_pat(pulse_idx)/2+num_bits_pulse)*oversamp_IF+1:end)]; %跳数据部分
...
% 根据跳频图案对应的频点将对应通道的波形滤波至零中频
% wav_temp(采样率1024) -> rx_pulse_mat(采样率64)
rx_pulse_mat_FNL = downConv_2MLL(Wav_temp_FNL, num_pulses, fh_pat, th_pat); %这个滤波函数不动？  

% 预取前后24bit位置的波形 等待后续处理
D_S1 = rx_pulse_mat_FNL(:,1:num_bits_pn*oversamp_BB);  %若是把上面做成了模块，这里就也要改
D_S1_one = D_S1(:,4:oversamp_BB:end);
...
rx_pulse_mat_pre = downConv_th(Wav_store_FNL(:,:), num_pulses, fh_pat, th_pat, 1);  % function downConv_th 只用于2M@B模式（前跳时数据部分）
...
counter_f_mid = repmat(delta_f, [num_pulses, 1]) * 2 * pi * t2(16:oversamp_IF/oversamp_BB:280*oversamp_IF) * T; %用16？原来是4
...
rx_pulse_mat_FNL2 = cell(num_pulses, 1); %这样用cell？
for j = 1:num_pulses
    th = th_pat(j)/2;
